...
if [ -z $AWS_SECRET_ACCESS_KEY ]; then
    echo 'You must define $AWS_SECRET_ACCESS_KEY'
    SETUP_PASS=0
fi
...
ec2-upload-bundle --retry -b $BUCKET -a $AWS_ACCESS_KEY -s $AWS_SECRET_ACCESS_KEY -m ${TEMPDIR}/${IMAGE_NAME}.manifest.xml
if [ $? -ne 0 ]; then
...
ec2-register --sriov simple -O $AWS_ACCESS_KEY -W $AWS_SECRET_ACCESS_KEY -n ${IMAGE_NAME} -d ${IMAGE_NAME} ${BUCKET}/${IMAGE_NAME}.manifest.xml $VIRT_ARGS
